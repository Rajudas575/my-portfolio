...
import { Dam, Menu, Moon, Sun, X } from "lucide-react";
          ...
          <div className="hidden lg:flex items-center space-x-6">
            {navItems.map((item) => (
              <a
                ...
                className="relative">
                <motion.span
                  className={`font-medium transition-colors duration-300 ${activeSection === item.name.toLocaleLowerCase() ? colors.textActive : `${colors.textSecondary} hover:text-yellow-500`}`}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}>
                  {item.name}
                </motion.span>
          ...
          <div className="flex lg:hidden items-center space-x-4 px-2">
            <motion.button
              ...
              {isMenuOpen ? (
                ...
                <Menu
                  className={`w-5 h-5 ${darkMode ? "text-white" : "text-gr-"}`}
                />
              )}
            </motion.button>
          </div>
          ...
          <motion.div
            ...
            <div className="px-4 py-3 space-y-2">
              ...
              <motion.a
                ...
                whileTap={{ scale: 0.9 }}
                className={`block py-3 px-4 text-center font-semibold rounded-lg bg-linear-r ${colors.button} text-white shadow-md`}
                data-aos="fade-up"
                data-aos-delay="800">
                Hire Me
              </motion.a>
